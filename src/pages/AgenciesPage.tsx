                ...
                <TabsList className="grid w-full grid-cols-3 mb-8">
                    {categories.map((cat) => (
                        <TabsTrigger key={cat.id} value={cat.id} className="flex items-center gap-2">
                            {(() => {
                                const I = CATEGORY_ICONS[cat.icon_name] ?? Scale;
                                return <I className="w-4 h-4 hidden md:inline"/>;
                            })()}
                            {cat.label_zh}
                        </TabsTrigger>
                    ))}
                ...
                <TabsContent value={activeCategory} className="mt-0">
                    ...
                    <div className="space-y-4 mb-6">
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Button variant={sceneId === null ? 'default' : 'outline'} size="sm" onClick={() => {
                                setSceneId(null);
                                setPage(1);
                            }}>
                                全部场景
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {agencies.map((agency) => (
                                    <div key={agency.id}
                                         className="flex items-start gap-3 p-4 rounded-lg bg-card border border-border hover:border-primary/30 hover:bg-primary/5 transition-all cursor-pointer"
                                         onClick={() => {
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                ...
                                                <Badge variant="secondary"
                                                       className="text-[10px] h-5 px-1.5">{sceneLabel(agency.scene_id)}</Badge>
                                            </div>
                                            <div className="text-xs text-muted-foreground space-y-0.5">
                                                <div className="flex items-center gap-1"><MapPin
                                                    className="w-3 h-3 shrink-0"/><span>{agency.region}</span></div>
                            ...
                            {agencies.length === 0 && (
                                <div className="text-center py-12"><p
                                    className="text-muted-foreground">暂无匹配机构，请调整筛选条件</p></div>
                            )}
                            {totalPages > 1 && (
                        ...
                        <>
                            ...
                            <div className="space-y-5 py-2">
                                {selectedAgency.highlight && <Badge
                                    className={`${getHighlightVariant(selectedAgency.highlight)} text-xs`}>{selectedAgency.highlight}</Badge>}
                                <div className="flex items-start gap-3"><MapPin
                                    className="w-4 h-4 text-primary mt-0.5"/>
                                    <div><p className="text-xs text-muted-foreground">覆盖区域</p><p
                                        className="text-sm font-medium">{selectedAgency.region}</p></div>
                                </div>
                                <div className="flex items-start gap-3"><Phone className="w-4 h-4 text-primary mt-0.5"/>
                                    <div><p className="text-xs text-muted-foreground">联系电话</p><p
                                        className="text-sm font-medium">{selectedAgency.phone}</p></div>
                                </div>
                                <div className="flex items-start gap-3"><Mail className="w-4 h-4 text-primary mt-0.5"/>
                                    <div><p className="text-xs text-muted-foreground">联系邮箱</p><p
                                        className="text-sm font-medium">{selectedAgency.email}</p></div>
                                </div>
                                <div className="flex items-start gap-3"><Briefcase
                                    className="w-4 h-4 text-primary mt-0.5"/>
                                    <div><p className="text-xs text-muted-foreground">主攻业务</p><p
                                        className="text-sm">{selectedAgency.business}</p></div>
                                </div>
                                <div className="bg-primary/5 border border-primary/10 rounded-lg p-4">
                                    <div className="flex items-start gap-3"><Target
                                        className="w-4 h-4 text-primary mt-0.5"/>
                                        <div><p className="text-xs text-muted-foreground">核心优势</p><p
                                            className="text-sm font-medium">{selectedAgency.advantage}</p></div>
                                    </div>
                                </div>
                            </div>
                        </>
