...
export default function AdminAgenciesPage() {
  ...
  const handleSave = async () => {
    ...
    if (editId) { await api.put(`/admin/agencies/${editId}`, form); }
    else { await api.post('/admin/agencies', form); }
    setDlgOpen(false); fetchList();
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b text-left text-muted-foreground">
                <th className="p-3">名称</th><th className="p-3">场景</th><th className="p-3">区域</th><th className="p-3">状态</th><th className="p-3">操作</th>
              </tr></thead>
              <tbody>
                {items.map((item: any) => (
      ...
      <Dialog open={dlgOpen} onOpenChange={setDlgOpen}>
        <DialogContent className="max-w-lg max-h-[90dvh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            <div><Label>名称</Label><Input value={form.name_zh} onChange={(e) => setForm((p) => ({ ...p, name_zh: e.target.value }))} /></div>
            <div><Label>场景</Label>
              <Select value={form.scene_id} onValueChange={(v) => setForm((p) => ({ ...p, scene_id: v }))}>
                ...
                <SelectContent>{scenes.map((s: any) => <SelectItem key={s.id} value={s.id}>{s.label_zh}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div><Label>区域</Label><Input value={form.region} onChange={(e) => setForm((p) => ({ ...p, region: e.target.value }))} /></div>
            <div><Label>电话</Label><Input value={form.phone} onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))} /></div>
            <div><Label>邮箱</Label><Input value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} /></div>
            <div><Label>业务</Label><Input value={form.business} onChange={(e) => setForm((p) => ({ ...p, business: e.target.value }))} /></div>
            <div><Label>优势</Label><Input value={form.advantage} onChange={(e) => setForm((p) => ({ ...p, advantage: e.target.value }))} /></div>
            <div><Label>亮点标签</Label><Input value={form.highlight} onChange={(e) => setForm((p) => ({ ...p, highlight: e.target.value }))} /></div>
            <Button className="w-full" onClick={handleSave} disabled={!form.name_zh || !form.scene_id}>保存</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
