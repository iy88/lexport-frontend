...
const empty = { type: 'cooperation', title: '', country_id: '', source: '', date: '', summary: '', risk_level: '', involved_laws: '', response: '', update_type: '', change_desc: '', impact: '', advice: '' };
...
export default function AdminNewsPage() {
  ...
  const handleSave = async () => {
    ...
    if (form.type === 'hotspot') { Object.assign(body, { risk_level: form.risk_level, involved_laws: form.involved_laws, response: form.response }); }
    if (form.type === 'update') { Object.assign(body, { update_type: form.update_type, change_desc: form.change_desc, impact: form.impact, advice: form.advice }); }
    if (editId) { await api.put(`/admin/news/${editId}`, body); } else { await api.post('/admin/news', body); }
    setDlgOpen(false);
    fetchList();
  };
    ...
    <div className="flex flex-col flex-1 min-h-0">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b text-left text-muted-foreground">
                <th className="p-3">标题</th><th className="p-3">类型</th><th className="p-3">国家</th><th className="p-3">状态</th><th className="p-3">操作</th>
              </tr></thead>
              <tbody>
                {items.map((item: any) => (
                  <tr key={item.id} className="border-b hover:bg-muted/30">
                    <td className="p-3 font-medium">{item.title}</td>
                    <td className="p-3"><Badge variant="outline" className="text-xs">{types.find((t) => t.value === item.type)?.label_zh}</Badge></td>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 pt-4 shrink-0">
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => <Button key={p} variant={p === page ? 'default' : 'outline'} size="sm" className="w-9 h-9" onClick={() => setPage(p)}>{p}</Button>)}
          ...
          <div className="space-y-4 py-2">
            <div><Label>类型</Label>
              <Select value={form.type} onValueChange={(v) => setForm((p) => ({ ...p, type: v }))}>
                ...
                <SelectContent>{types.map((t) => <SelectItem key={t.value} value={t.value}>{t.label_zh}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            ...
            <div><Label>来源</Label><Input value={form.source} onChange={(e) => setForm((p) => ({ ...p, source: e.target.value }))} /></div>
            <div><Label>日期</Label><Input type="date" value={form.date} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} /></div>
            <div><Label>摘要</Label><Input value={form.summary} onChange={(e) => setForm((p) => ({ ...p, summary: e.target.value }))} /></div>
            {form.type === 'hotspot' && <>
              <div><Label>风险等级</Label><Select value={form.risk_level} onValueChange={(v) => setForm((p) => ({ ...p, risk_level: v }))}><SelectTrigger><SelectValue placeholder="选择" /></SelectTrigger><SelectContent><SelectItem value="high">高</SelectItem><SelectItem value="medium">中</SelectItem><SelectItem value="low">低</SelectItem></SelectContent></Select></div>
              <div><Label>涉事法规</Label><Input value={form.involved_laws} onChange={(e) => setForm((p) => ({ ...p, involved_laws: e.target.value }))} /></div>
              <div><Label>应对建议</Label><Input value={form.response} onChange={(e) => setForm((p) => ({ ...p, response: e.target.value }))} /></div>
            </>}
            {form.type === 'update' && <>
              <div><Label>更新类型</Label><Select value={form.update_type} onValueChange={(v) => setForm((p) => ({ ...p, update_type: v }))}><SelectTrigger><SelectValue placeholder="选择" /></SelectTrigger><SelectContent><SelectItem value="修订">修订</SelectItem><SelectItem value="新增">新增</SelectItem><SelectItem value="废止">废止</SelectItem></SelectContent></Select></div>
              <div><Label>变更描述</Label><Input value={form.change_desc} onChange={(e) => setForm((p) => ({ ...p, change_desc: e.target.value }))} /></div>
              <div><Label>影响</Label><Input value={form.impact} onChange={(e) => setForm((p) => ({ ...p, impact: e.target.value }))} /></div>
              <div><Label>建议</Label><Input value={form.advice} onChange={(e) => setForm((p) => ({ ...p, advice: e.target.value }))} /></div>
            </>}
