...
import {
  AlertTriangle, Globe, Search, ChevronLeft, ChevronRight, Loader2,
} from 'lucide-react';

const PAGE_SIZE = 6;

interface Law {
  id: number; title: string; country_id: string; scene_id: string;
  level: string; penalty: string; effective_date: string; summary: string;
}
...
export default function LawsPage() {
  ...
  const handleCountryChange = (id: string | null) => {
    setCountryId(id); setSceneId(null); setPage(1); setKeyword('');
  };
  const handleSceneChange = (id: string | null) => { setSceneId(id); setPage(1); };
  const handleSearch = () => { setPage(1); fetchLaws(); };
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={countryId === null ? 'default' : 'outline'} size="sm" onClick={() => handleCountryChange(null)}>
            全部国家
          </Button>
          {countries.map((c) => (
            <Button key={c.id} variant={countryId === c.id ? 'default' : 'outline'} size="sm" onClick={() => handleCountryChange(c.id)}>
              {c.name_zh}
            </Button>
          ))}
        </div>
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant={sceneId === null ? 'default' : 'outline'} size="sm" onClick={() => handleSceneChange(null)}>
            全部场景
          </Button>
          {scenes.map((s) => (
            <Button key={s.id} variant={sceneId === s.id ? 'default' : 'outline'} size="sm" onClick={() => handleSceneChange(s.id)}>
              {s.label_zh}
            </Button>
          ))}
        </div>
              ...
              <Card key={law.id} className="shadow-card">
                <CardContent className="p-5">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-3">
                    ...
                    {(() => { const s = scenes.find((sc) => sc.id === law.scene_id); return s ? <Badge className="bg-primary/10 text-primary border-primary/20 text-xs">{s.label_zh}</Badge> : null; })()}
                  </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <Button key={p} variant={p === page ? 'default' : 'outline'} size="sm" className="w-9 h-9" onClick={() => setPage(p)}>
                  {p}
                </Button>
              ))}
